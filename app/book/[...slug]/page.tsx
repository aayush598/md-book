...
export default function BookPage() {
  ...
  // Derive bookId and config from slug
  const { bookId, config } = useMemo(() => {
    if (slug.length === 0) return { bookId: null, config: null };
    const gh = parseBookSlug(slug);
    if (gh) {
      return {
        bookId: gh.bookId,
        config: {
          ...
          repo: gh.config.repo,
          branch: gh.config.branch,
          path: gh.config.path,
          name: gh.config.path
            ? `${gh.config.owner}/${gh.config.repo}/${gh.config.path}`
    ...
    getBookTree(config)
      .then((b) => {
        ...
        if (fileParam) {
          ...
          if (saved && flat.some((f) => f.path === saved.filePath)) {
            startPath = saved.filePath;
          }
        }
        if (!startPath && flat.length > 0) {
  ...
  useEffect(() => {
    if (bookId && anchorFile) {
      saveReadingProgress(bookId, anchorFile);
    }
  }, [bookId, anchorFile]);
  ...
  const handleFileSelect = useCallback(
    (_chapter: unknown, filePath: string) => {
      ...
      const path = `/book/${slug.join("/")}?file=${encodeURIComponent(filePath)}`;
      if (readingMode === "page") {
        setTimeout(() => {
          router.push(path);
  ...
  const toggleBookmark = useCallback(() => {
    ...
    if (bookmarkActive) {
      removeBookmark(bookId, anchorFile);
    } else {
      addBookmark({
        bookId,
        bookName: book.name,
        filePath: anchorFile,
        title: normalizeName(anchorFile.split("/").pop() || ""),
        timestamp: Date.now(),
      });
    }
    // Force re-render
    ...
    return (
      <div className="flex h-screen items-center justify-center" style={{ background: "var(--bg-page)" }}>
        <div className="text-center">
          ...
          <Link href="/" className="mt-2 inline-block text-xs" style={{ color: "var(--accent)" }}>Go back home</Link>
        </div>
      </div>
    );
  }
      ...
      >
        <div className="flex items-center gap-3 px-4 py-2.5">
          ...
          <div className="flex items-center gap-2 min-w-0">
            <Link href="/" className="flex items-center gap-1.5 text-xs font-medium transition-colors" style={{ color: "var(--text-tertiary)" }}
          ...
          <div className="ml-auto flex items-center gap-1 sm:gap-2">
            ...
            <button
              ...
              style={{
                ...
                background: flashcardMode ? "var(--accent-bg)" : "transparent",
              }}
              onMouseEnter={(e) => !flashcardMode && (e.currentTarget.style.background = "var(--bg-hover)")}
              onMouseLeave={(e) => !flashcardMode && (e.currentTarget.style.background = "transparent")}
              title={flashcardMode ? "Exit flashcards" : "Flashcards"}
            >
              <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-lg transition-colors"
              style={{
                color: bookmarkActive ? "var(--accent)" : "var(--text-tertiary)",
                background: bookmarkActive ? "var(--accent-bg)" : "transparent",
              }}
              onMouseEnter={(e) => !bookmarkActive && (e.currentTarget.style.background = "var(--bg-hover)")}
              onMouseLeave={(e) => !bookmarkActive && (e.currentTarget.style.background = "transparent")}
              title={bookmarkActive ? "Remove bookmark" : "Add bookmark"}
            >
              <svg className="h-4 w-4" fill={bookmarkActive ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {currentIdx !== -1 && (
              <div className="flex items-center gap-1 text-xs font-medium" style={{ color: "var(--text-tertiary)" }}>
                ...
                <span className="hidden sm:inline">/ {allFiles.length}</span>
                {readingTime > 0 && (
                  <span className="hidden sm:inline text-nowrap" style={{ color: "var(--text-muted)" }}>
                    <span className="mx-0.5">·</span>{readingTime}m
                  </span>
                )}
              </div>
            )}
